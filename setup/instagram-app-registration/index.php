...
{
	set_metadata('instagram-client-id', $_POST['client-id']);
	set_metadata('instagram-client-secret', $_POST['client-secret']);
...
if (isset($_GET['code']))
{
	$clientID = get_metadata('instagram-client-id');
	$clientSecret = get_metadata('instagram-client-secret');
	...
	set_metadata('instagram-access-token', $accessToken);
	...
	<body>
		<div class="container">
			<div class="jumbotron">
				...
				<ol>
					<li>
						Once logged in, navigate to <a target="_blank" href="http://instagram.com/developer/">instagram.com/developer</a>
					</li>
					<li>
						In the top-right of the screen, click on <b>Manage Clients</b>, then click <b>Register New Client.</b> If you haven't done so already, you may also need to agree to the Developer License Agreement.
						<img class="tut-screen-image img-rounded" src="tut_screen_1.jpg" />
					</li>
					<li>
						You will then be taken to the following page where you would need to register Museum Now with the Instagram API:
						<img class="tut-screen-image img-rounded" src="tut_screen_2.jpg" />
						Copy and paste from the following fields into the above page. Once done, click <b>Register</b>.
						<form role="form">
							<div class="form-group">
								<label>Application Name:</label>
								<input type="text" class="form-control" value="Museum Now : <?php echo get_metadata('institution-name'); ?>" readonly="readonly">
							</div>
							<div class="form-group">
								<label>Description:</label>
								<input type="text" class="form-control" value="A live public feed for <?php echo get_metadata('institution-name'); ?>" readonly="readonly" />
							</div>
							<div class="form-group">
								<label>Website:</label>
								<input type="text" class="form-control" value="<?php echo get_metadata('digital-sign-ref-url'); ?>" readonly="readonly" />
							</div>
							<div class="form-group">
								<label>OAuth redirect_uri:</label>
								<input type="text" class="form-control" value="<?php echo get_metadata('museum-now-root')."setup/instagram-app-registration"; ?>" readonly="readonly" />
							</div>
						</form>
					</li>
					<li>
						The next page gives you a Client ID and a Client Secret. Copy and paste these into the fields below:
						...
						<form role="form" method="POST" action="./">
							...
							You will then be taken to Instagram.com where you will be asked to authenticate your app.
							<div class="form-group">
